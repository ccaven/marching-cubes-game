...
function noise2D (x, y, octaves=4) {
    let scl = 1.0, v = 0.0;
    for (let i = 0; i < octaves; i ++) {
        v += noiseGenerator.noise2D(x * scl, y * scl) / scl * 0.5;
        scl *= 2.0;
    }
    return v;
}
...
function noise3D (x, y, z, octaves=4) {
    let scl = 1.0, v = 0.0;
    for (let i = 0; i < octaves; i ++) {
        v += noiseGenerator.noise3D(x * scl, y * scl, z * scl) / scl * 0.5;
        scl *= 2.0;
    }
    return v;
}

function noise4D (x, y, octaves=4) {
    let scl = 1.0, v = 0.0;
    for (let i = 0; i < octaves; i ++) {
        v += noiseGenerator.noise4D(x * scl, y * scl, z * scl, w * scl) / scl * 0.5;
        scl *= 2.0;
    }
    return v;
}
